		...
		enum abox_dbg_dump_src src, const char *reason)
{
	...
	if (p_abox_dbg_dump) {
		...
		abox_gicd_dump(data->dev_gic, (char *)p_dump->sfr_gic_gicd,
				0, sizeof(p_dump->sfr_gic_gicd));
	} else if (p_abox_dbg_dump_min) {
		struct abox_dbg_dump_min *p_dump = &(*p_abox_dbg_dump_min)[src];
		...
		const char *reason)
{
	...
	abox_gicd_dump(data->dev_gic, (char *)abox_dump_simple.sfr_gic_gicd,
			0, sizeof(abox_dump_simple.sfr_gic_gicd));
}
...
void abox_dbg_dump_suspend(struct device *dev, struct abox_data *data)
{
	...
	abox_gicd_dump(data->dev_gic, (char *)abox_dump_suspend.sfr_gic_gicd,
...
static BIN_ATTR(calliope_dram, 0440, calliope_dram_read, NULL, DRAM_FIRMWARE_SIZE);
